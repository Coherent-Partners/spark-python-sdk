...
__all__ = ['AsyncHistory']


class AsyncHistory(AsyncApiResource):
    @property
    def downloads(self) -> 'AsyncLogDownload':
        return AsyncLogDownload(self.config, self._client)
    ...
    async def rehydrate(
        ...
        *,
        ...
        return (await self.request(download_url)).copy_with(data=response.data)
    ...
    async def download(
        ...
        *,
        ...
        response = await downloads.initiate(
            folder=folder,
            service=service,
            type=type,
        ...
        job = await downloads.get_status(
            job_id=job_id,
            folder=folder,
            service=service,
            type=type,
        ...
        logs = await self.request(download_url)
...
class AsyncLogDownload(AsyncApiResource):
    async def initiate(
        self,
        *,
        folder: str,
        ...
        *,
        ...
        while True:
            ...
            if progress < 100 and retries < max_retries:
                ...
                retries += 1
                await asyncio.sleep(get_retry_timeout(retries, retry_interval))
                response = await self.request(url)
            else:
