...
class AsyncFolders(AsyncApiResource):
    ...
    async def create(
        ...
        *,
        ...
        response = await self.request(url, method='POST', form=form)

        if response.status == 200 and isinstance(response.data, dict) and response.data['status'] == 'Success':
            if cover:
                await self.upload_cover(response.data['data']['folderId'], cover)
            return await self.request(response.data['data']['get_product_url'])
    ...
    async def update(
        ...
        *,
        ...
        if cover:
            await self.upload_cover(id, cover)
...
class AsyncCategories(AsyncApiResource):
    ...
    async def list(self):
        return await self.request(Uri.of(None, endpoint='lookup/getcategories', **self.base_uri))

    async def save(self, name: str, *, key: Optional[str] = None, icon: Optional[str] = None):
    ...
    async def delete(self, key: str):
        ...
        response = await self.request(url, method='DELETE')
        return response.copy_with(data=self.__extract_data(response.data))
